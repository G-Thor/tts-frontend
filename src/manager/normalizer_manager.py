...
def extract_prenorm_tuples(prenorm_arr, sent):
    """
...
def normalize(text: str) -> Tuple:
    ...
    prenorm_tuples = extract_prenorm_tuples(abbr_sent, text)
...
def normalize_token_list(token_list: list) -> list:
    ...
    pre_normalized, final_normalized = normalize(text)
    norm_index = 0
    ...
    for i, tok in enumerate(token_list):
        ...
        if isinstance(tok, TagToken):
            ...
            if normalized_base_token == original_token and len(final_normalized[norm_index][1].split()) > 1:
                for wrd in final_normalized[norm_index][1].split():
                    ...
                    if wrd.endswith(',') or wrd.endswith('.'):
                        ...
                        wrd = wrd[:-1]
                    if wrd:
                        norm_tok = init_normalized(tok, wrd,
                ...
                for j, wrd in enumerate(pre_normalized[i][1].split()):
                    if final_normalized[norm_index+j][0] == wrd:
                        ...
                        if norm_wrd.endswith(',') or wrd.endswith('.'):
                            ...
                            norm_wrd = norm_wrd[:-1]
                        if norm_wrd:
                            norm_tok = init_normalized(tok, norm_wrd,
            ...
            # no change in normalization
                ...
                norm_tok = init_normalized(tok, final_normalized[norm_index][1],
                                           final_normalized[norm_index][2])
                normalized_tokens.append(norm_tok)
