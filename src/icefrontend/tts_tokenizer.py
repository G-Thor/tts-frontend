...
class Tokenizer:
    ...
    def should_process(self, token: str) -> bool:
        ...
        if re.match(
            r"(millj./)|(.+/klst)|(.+/kwst)|(.+/gwst)|(.+/gw\\.st)|(.+/mwst)|(.+/twst)|(.+/m²)|(.+/m³)"
            r"|(.+/mm²)|(.+/mm³)|(.+/cm²)|(.+/cm³)|(.+/ferm)",
            token.lower(),
        ):
            return False
    ...
    def is_uppercase_abbr(self, token: str) -> bool:
        return re.match("(" + UPPER_CASE + r"\.)+", token) and not self.is_abbreviation(
            token
        )
    ...
    def is_abbreviation(self, token: str) -> bool:
        ...
        if (
            token.lower() in self.abbreviations
            or token.lower() + "." in self.abbreviations
        ):
            return True
