...
class Phrasing:
    ...
    def __init__(self, pause_labels=None):
        if pause_labels is None:
            ...
            ValueError(
                "An argument containing pause labels must either be a label as string or "
    ...
    def insert_pauses(self, text: list) -> list:
        ...
        if not self.is_valid_input(text):
            ValueError(
                "Input to phrasing needs to be a list of strings in IceParser format"
            )
    ...
    def is_valid_input(self, text: list) -> bool:
        ...
        if not re.match("\[.+(" + NP + "|" + VP + ").+]", sent):
            logging.error(
                "It seems that the phrasing input is not in IceParser format: '"
                + str(sent)
                + "'"
            )
            return False
    ...
    def list_phrases(self, line: str) -> list:
        ...
        clean_phrases = [
            re.sub(PHRASE_CLOSING, "", item).rstrip(" ") for item in phrase_list
        ]
            ...
            for j in range(len(phrased_sentences[i])):
                try:
                    ...
                    if phrased_sentences[i][j - 1][-1] in PAUSE_PUNCTUATION:
                        ...
                        if self.long_pause_criteria_previous_phrase_met(
                            phrased_sentences[i], j
                        ):
    ...
    def pause_criteria_not_met(self, phrased_sentence: list, phrase_index: int) -> bool:
        return (
            phrased_sentence[phrase_index][0].startswith(
                (NP, AP, PP, VP, INJP, MWE, ADVP)
            )
            or (
                phrased_sentence[0] == CP
                and phrased_sentence[phrase_index - 2][0] in [NP_SEQUENCE, AP_SEQUENCE]
            )
            or (
                phrased_sentence[phrase_index][0] == CP
                and (
                    phrased_sentence[phrase_index - 1][0] == PP
                    and phrased_sentence[phrase_index + 1][0] == PP
                )
            )
        )

    def medium_pause_criteria_met(
        self, phrased_sentence: list, phrase_index: int
    ) -> bool:
        if (
            phrased_sentence[phrase_index][1] == REL
            and phrased_sentence[phrase_index - 1][0] == NP
        ):
            if phrased_sentence[phrase_index - 2][0] == PP or (
                phrased_sentence[phrase_index - 2][0] == NP
                and phrased_sentence[phrase_index - 3][0] == PP
            ):
                return True

        if (
            phrased_sentence[phrase_index][0] == CP
            and not phrased_sentence[phrase_index - 1][-1].startswith(TF)
        ) or (
            phrased_sentence[phrase_index - 2][0] == PP
            and phrased_sentence[phrase_index - 1][0] == NP
            and phrased_sentence[phrase_index][0] == SCP
            and not phrased_sentence[phrase_index][1] == REL
        ):
            return True

        return False

    def long_pause_criteria_met(
        self, phrased_sentence: list, phrase_index: int
    ) -> bool:
        return (
            phrased_sentence[phrase_index][0] == SCP
            and not phrased_sentence[phrase_index][1] == REL
        )

    def long_pause_criteria_previous_phrase_met(
        self, phrased_sentence: list, phrase_index: int
    ) -> bool:
    ...
    def extract_text(self, phrased_sentence: list) -> str:
        ...
        for i in range(len(clean_phrases)):
            extracted_text_list.append(
                [
                    clean_phrases[i][index]
                    for index in range(1, len(clean_phrases[i]), 2)
                ]
            )
    ...
    def switch_punctuation(self, clean_list):
        ...
        clean_list3 = [
            re.sub("[,–\-:/−]", self.medium_pause, sent) for sent in clean_list2
        ]
        clean_list4 = [
            re.sub('[„“()"”·*]', self.short_pause, sent) for sent in clean_list3
        ]
        return clean_list4
