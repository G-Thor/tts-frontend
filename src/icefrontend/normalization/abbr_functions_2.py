...
def replace_abbreviations(sent, domain):
    ...
    sent = replace_all(sent, denominator_dict, r"\/")
    ...
    if domain == 'other':
        sent = re.sub(r'(\W|^)(\d{1,3})(\/)(0|\d{1}\d+)(\W|$)', '\g<1>\g<2> \g<3> \g<4>\g<5>', sent)
        sent = re.sub(r'(\W|^)(\d{3}\d+)(\/)(\d+)(\W|$)', '\g<1>\g<2> \g<3> \g<4>\g<5>', sent)
    if domain == 'sport':
        sent = re.sub(r'(W|^)(\d{2}\d+)(\/)(\d+)(\W|$)', '\g<1>\g<2> \g<3> \g<4>\g<5>', sent)
        sent = re.sub(r'(W|^)(\d+)(\/)(\d{2}\d+)(\W|$)', '\g<1>\g<2> \g<3> \g<4>\g<5>', sent)
    return sent
