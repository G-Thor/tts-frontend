...
class Tokenizer:
    ...
    def finish_sentence(
        self, sentences: list, tmp_string: str, last_token: str
    ) -> None:
    ...
    def update_tmp_string(
        self, sentences: list, tmp_string: str, tokenized: str
    ) -> str:
    ...
    def check_last_token(
        self, sentences: list, tmp_string: str, last_token: str, tokenized: str
    ) -> str:
        if last_token:
    ...
    def is_full_stop_EOS(self, current: str, last: str) -> bool:
        ...
        if current[0].isupper() or current[0] == '"':
            return (
                not self.is_uppercase_abbr(last)
                and last.lower() not in self.abbreviations_non_ending
            )
        return False
    ...
    def process_special_characters(self, token: str) -> str:
        ...
        insert_space_after_anywhere = r"([\(\[\{])"
        # insert space before these symbols: ')', '[', '}' TODO: shouldn't '[' be ']' ?
        insert_space_before_anywhere = r"([\)\]\}])"
        ...
        # Replacements
        processed_token = re.sub(
            insert_space_after_anywhere, r"\g<1> ", processed_token
        )
        processed_token = re.sub(
            insert_space_before_anywhere, r" \g<1>", processed_token
        )
        processed_token = re.sub(
            insert_space_after_if_beginning, r"\g<1> \g<2>", processed_token
        )
        processed_token = re.sub(
            insert_space_before_if_end, r"\g<1> \g<2>", processed_token
        )
        processed_token = re.sub(
            insert_space_before_if_end_and_punct, r"\g<1> \g<2>\g<3>", processed_token
        )
    ...
    def should_process(self, token: str) -> bool:
        ...
        if re.match(
            "(millj./)|(.+/klst)|(.+/kwst)|(.+/gwst)|(.+/gw\\.st)|(.+/mwst)|(.+/twst)|(.+/m²)|(.+/m³)"
            "|(.+/mm²)|(.+/mm³)|(.+/cm²)|(.+/cm³)|(.+/ferm)|(.+/%)",
            token.lower(),
        ):
            return False
    ...
    def is_uppercase_abbr(self, token: str) -> bool:
        return re.match("(" + UPPER_CASE + "\\.)+", token) and not self.is_abbreviation(
            token
        )
