...
symb_ptrn = r"[^A-ZÁÐÉÍÓÚÝÞÆÖa-záðéíóúýþæö\s]"
...
def handle_sentence_tokenwise(sent, domain, tagger):
    ...
    dashsent = replace_domain(sentsplit, domain)
    tagsent = tagger.tag_sent(dashsent)
    print(list(zip(dashsent, tagsent)))
    split_zip = list(zip(sentsplit, list(tagsent[1:]) + [""], tagsent))
    for word, nexttag, currtag in split_zip:
        norm_word = word
        if re.match(r"[\d½⅓¼⅔¾\-\–]", norm_word):
        ...
        returnsent.append((word, norm_word, currtag))
