...
for rule, string, number in tp.ones_zip:
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + number + r"\d*$"), rule, "points", " komma" + string)
    )
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + r"\d" + number + r"\d*$"), rule, "point2", string)
    )
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + r"\d{2}" + number + r"\d*$"), rule, "point3", string)
    )
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + r"\d{3}" + number + r"\d*$"), rule, "point4", string)
    )
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + r"\d{4}" + number + r"\d*$"), rule, "point5", string)
    )
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + r"\d{5}" + number + r"\d*$"), rule, "point6", string)
    )
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + r"\d{6}" + number + r"\d*$"), rule, "point7", string)
    )
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + r"\d{7}" + number + r"\d*$"), rule, "point8", string)
    )
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + r"\d{8}" + number + r"\d*$"), rule, "point9", string)
    )
    decimal_thousands_tuples.append(
        ((npa.zeropnt_ptrn + r"\d{9}" + number + r"\d*$"), rule, "point10", string)
    )
...
for rule, string, number in tp.ones_zip:
    decimal_small_tuples.append(
        ((npa.zeropnt_ptrn + r"\d" + number + r"\d*$"), rule, "points_ones", string)
    )
    decimal_small_tuples.append(
        ((npa.zeropnt_ptrn + number + r"$"), rule, "points_ones", string)
    )

for string, number in tp.tens_zip:
    decimal_small_tuples.append(
        ((npa.zeropnt_ptrn + number + r"\d*$"), ".*", "points_tens", string)
    )

for string, number in tp.dozens_zip:
    decimal_small_tuples.append(
        ((npa.zeropnt_ptrn + number + r"0$"), ".*", "points_dozens", string)
    )
    decimal_small_tuples.append(
        ((npa.zeropnt_ptrn + number + r"[1-9]$"), ".*", "points_dozens", string + " og")
    )
