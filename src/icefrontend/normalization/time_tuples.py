...
for string, number in tp.ones_zip_time:
    ...
    time_tuples.append(
        ("^([01]?[0-9]|2[0-4])[:\.][02-5]" + number + "$", ".*", "second_one", string)
    )
...
for string, number in tens_zip_time:
    ...
    time_tuples.append(
        ("^([01]?[0-9]|2[0-4])[:\.]" + number + "$", ".*", "second_ten", string)
    )
    time_tuples.append(("^2[1-4][:\.][0-5]\d$", ".*", "first_ten", "tuttugu og"))

for string, number in tp.dozens_zip[:4]:
    time_tuples.append(
        ("^([01]?\d|2[0-4])[:\.]" + number + "0$", ".*", "second_ten", string)
    )
    time_tuples.append(
        (
            "^([01]?\d|2[0-4])[:\.]" + number + "[1-9]$",
            ".*",
            "second_ten",
            string + " og",
        )
